...
//	void setRoot(int x) {
//		if (!isEmpty()) {
//			cout << "Root already exists. " << endl;//if tree is not empty it will print the msg not empty//
//
//			return;
//		}
//		else {// tree is empty it will create a new root node//
//			root = makeTree(x);
//		}
//	}
...
//		child->parent = p;//The child node has a pointer called parent(p)
//		p->left = child;//The p node (which is the parent) has a pointer called left.p has left child called child//
//	}
//
...
//		if (index==-1 ||index >= NUMNODES || BT[index].info == -1) return;
